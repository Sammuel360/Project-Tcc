    ...
    public function inserir()
{
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $chamado->orgao_id = $orgao_id;
        $chamado->status = 'pendente'; // Status padrão
        $chamado->data_hora = date('Y-m-d H:i:s');

        // Tenta salvar o chamado
        if ($chamado->save()) {
            $lastInsertId = $chamado->getLastInsertId(); // Obtém o ID do último chamado inserido
